...
public class UDDINaming {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Collection<String> urls = instance.list(orgName);
		...
		String orgName="SD-Store-test";
		String wsUrl1="http://localhost:8082/store-ws/endpoint";
		String wsUrl2="http://localhost:8083/store-ws/endpoint";
		UDDINaming instance;
		Collection<String> urls=null;
		try {
			instance = new UDDINaming(uddiURL);
			instance.unbind(orgName);
			urls = instance.queryServiceBindings(orgName);
			System.out.printf("found %d urls%n",urls.size());
			for(String endpoint: urls){
				System.out.println(endpoint);
			}			
			instance.addServiceBinding(orgName, wsUrl1);
			urls = instance.queryServiceBindings(orgName);
			...
			if (urls != null){
				...
				System.out.println("found 0 urls");
			}
			instance.addServiceBinding(orgName, wsUrl2);
			urls = instance.queryServiceBindings(orgName);
			if (urls != null){
				System.out.printf("found %d urls%n",urls.size());
				for(String endpoint: urls){
	...
	public Collection<String> queryServiceBindings(String orgName)throws JAXRException{
		autoConnect();
		try {
			return queryServiceBindingsImpl(orgName);
		} finally {
			autoDisconnect();
		}
	}

	public  void addServiceBinding(String orgName,String url) throws JAXRException{
		autoConnect();
		try {
			addServiceBindingImpl(orgName,url);
		} finally {
			autoDisconnect();
		}
	}

	private Collection<String> queryServiceBindingsImpl(String orgName)throws JAXRException{
		...
		if(found){
			...
			@SuppressWarnings("unchecked")
			Collection<ServiceBinding> serviceBindings = service.getServiceBindings();
			for(ServiceBinding sb: serviceBindings){
				ret.add(sb.getAccessURI());
			}
		}    	
		return ret;
	}
	...
	private  void addServiceBindingImpl(String orgName,String url) throws JAXRException{
		...
		if(found){   
			...
			success = (response.getStatus() == JAXRResponse.STATUS_SUCCESS);

		}else{
			success = publish(orgName,url);        	  
		}
		if (debugFlag) {
			if (success) {
				System.out.println("Add service binding completed successfully.");
			} else {
				System.out.println("Add service binding error during registration.");
			}
		}    
	}
